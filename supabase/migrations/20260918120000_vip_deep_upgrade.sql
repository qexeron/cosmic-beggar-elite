-- ============================================================
-- TILANCHILIK.UZ — VIP DEEP UPGRADE
-- Yangi ustunlar (gif/video media, sticker, pin), ochiq donatlar,
-- adminda tahrirlanadigan matnlar, indekslar va realtime.
-- ============================================================

-- ---------- POSTS: gif / video / sticker / pin ----------
...
ALTER TABLE public.posts
  ...
  CHECK (media_kind IN ('image', 'gif', 'video', 'none'));

-- eski postlarda image_url bo'lsa — media_url ga ko'chiramiz
UPDATE public.posts
...
-- ---------- DONATIONS: ochiq (mehmon) donatlar ----------
...
ALTER TABLE public.donations
  ADD CONSTRAINT donations_source_check CHECK (source IN ('admin', 'public'));

ALTER TABLE public.donations
  DROP CONSTRAINT IF EXISTS donations_amount_sane;
ALTER TABLE public.donations
  ADD CONSTRAINT donations_amount_sane CHECK (amount >= 0 AND amount <= 1000000000);

-- mehmonlar faqat visible = true bo'lgan yozuvlarni ko'radi
...
-- ---------- FLOATERS: gif va sticker turlari ----------
ALTER TABLE public.floaters
  DROP CONSTRAINT IF EXISTS floaters_kind_check;
ALTER TABLE public.floaters
  ADD CONSTRAINT floaters_kind_check
  CHECK (kind IN ('emoji', 'meme', 'gif', 'sticker'));

-- ---------- SITE SETTINGS: matnlar admin qo'lida ----------
...
-- ---------- REALTIME (sayt o'zi yangilanib turishi uchun) ----------
DO $$
BEGIN
...
DO $$
BEGIN
  ALTER PUBLICATION supabase_realtime ADD TABLE public.floaters;
EXCEPTION WHEN duplicate_object THEN NULL; WHEN undefined_object THEN NULL;
END $$;
DO $$
BEGIN
